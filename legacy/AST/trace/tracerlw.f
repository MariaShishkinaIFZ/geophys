...
c
     ...
     *             pwflag,r1flag,qq,qqq,spn_start,
     *             bflag,crossed_flag,nbshot,ix2d,iy2d,m,ip,jp,kp,
     *             kc,ic,jc,just_reflected,n_first_av_cell,num_av_cell
      real         t(nxyzmx),du(nxyzmx),zb(nxymax),v1(nxyzmx),
     *             dum,iwght,ipwght,weight,max_dist
     ...
     *             xip,yjp,zkp,ray2(3)
      character*256 sfile,tinc_file,trlw_file,rfile,ufile,pfile,
...
c     read (*,*) addcov
      ...
      read (*,*) spn_start
      write (*,*) 'input the maximum shotpoint-receiver distance'
      read (*,*) max_dist
c
...
c     Open file for cos_r diagnostics
      open (40,file='cos_r.dat',status='unknown')
c
c
c
...
c
c     CONSIDER ONLY SHOTPOINTS INSIDE THE MAX_DIST CIRCLE
      if (dist>max_dist) goto 250
c
...
c
c     To make reflection angle more robust we will average it over num_av_cell cells
c     starting from n_first_av_cell after reflection
      n_first_av_cell=7
      num_av_cell=9
           if ((just_reflected.eq.n_first_av_cell)) then
...
c
            if ((just_reflected.gt.n_first_av_cell).and.
     *          (just_reflected.lt.n_first_av_cell+num_av_cell)) then
c           NOW FIND THE REFLECTION ANGLE
              v = sqrt(gradt(1)**2+gradt(2)**2+gradt(3)**2)
              gradb(1) = (gradb(1)-gradt(1)/v)
              gradb(2) = (gradb(2)-gradt(2)/v)
              gradb(3) = (gradb(3)-gradt(3)/v)
            end if
c
            if ((just_reflected.eq.n_first_av_cell+num_av_cell)) then
c           NOW FIND THE REFLECTION ANGLE
              v = sqrt(gradt(1)**2+gradt(2)**2+gradt(3)**2)
              gradb(1) = (gradb(1))/num_av_cell
              gradb(2) = (gradb(2))/num_av_cell
              gradb(3) = (gradb(3))/num_av_cell
...
c             SAVE FOUND VALUES TO FILE
     ...
     *	                      zbas,cos_r,cosn
              write (40,*) line,spn,xbas,ybas,zbas,dist,cos_r,dtdz
              just_reflected=0
            end if
c     Using for gradient estimation some cell above the reflection point
      if ((just_reflected.ne.0)) then
        just_reflected=just_reflected+1
      endif
c
...
c     of traces at the beginning of the file  
c     write (21,*) nshot
      do 505, qqq=1,nshot
        write (*,*) 'loop ',qqq
        read (30,*) qq
	write (21,*) spn_start+qq
	write (*,*) spn_start+qq,' of ',spn_start+nshot
 ...
 505  continue
c     Uncomment the following line to write out the number
c     of traces at the end of the file   
c     write (21,*) nshot
      close (21)
      close (30)
c
...
c
c     Uncomment the following line to write out the number
c     of reflection points at the beginning of the file  
c     write (31,*) nbshot
...
c        CALCULATE DT/DZ AND WRITE
     ...
     *        v1(iiiii+nx*ny+nx+1))/8.
         ...
         write (*,*) line,spn,xbas,ybas,zbas,cos_r,vel1,dtdz
         write (31,1070) spn_start+nshot,tt,iwght,line,spn,xbas,ybas,
     *	 		 zbas,cos_r,cosn,dtdz
c
 700  continue
c
      close (22)
...
c
      open (40,file='last_shot_rlw.n',status='unknown')
      write (40,*) spn_start+nbshot
      close (40)
c
 1010 format (a256)
 1020 format (1x,i2,i5,f7.3,f8.3)
 1030 format (1x,i3,i6,3f10.4,f15.10,f15.10,f15.10,f15.10)
 1040 format (1x,i7,i7,i7,i7,f12.5,f12.5,f12.5,f15.10,f15.10)
 1050 format (1x,i7,i7,i7,i7,f12.5,f12.5,f12.5,f10.5,f10.5,i5)
 1060 format (1x,i5,f10.5,f10.5,i5,i5,f10.5,f10.5,f10.5,f15.10,f15.10)
 1070 format (1x,i5,f10.5,f10.5,i5,i10,f13.5,f13.5,f13.5,f15.10,f15.10,
     *           f15.10)
c
      stop
      end
