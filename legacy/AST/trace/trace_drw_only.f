      program trace_drw_only
c	Modified to provide only path's of rays that dive deeper below the 
c       interface
...
c
     ...
     *		   cell_top
      character*80 sfile,tfile,pfile,tracefile,zfile,
...
c     Check if ray dive deeper below the interface (i.e. - not head wave)
      zbas=max(zb(nx*(j-1)+i),zb(nx*(j-1)+i+1),zb(nx*j+i),zb(nx*j+i+1))
      k_bas = (zbas-z0)/h+1
...
c     ------------------------------------
      do 505, qqq=nshot_start+1,nshot
        write (*,*) 'loop ',qqq
        read (30,*) qq
c     Will save ray only if it is deep diving wave trace
        if (hwshot(qq-nshot_start).eq.3) then
	  qqqq=qqqq+1
	  write (21,*) nshot_start+qqqq
	  write (*,*) qq,' of ',nshot,' numbered as ',nshot_start+qqqq
	else
	  write (*,*) qq, ' of ',nshot, ' is not DRW trace'
	endif
...
c        CALCULATE DT/DZ AND WRITE
         if (hwshot(qq-nshot_start).eq.3) then
           qqqq = qqqq + 1
           write (*,*) 'drw ray ',nshot_start+qqqq
           i = int((xbas-x0)/h) + 1
           j = int((ybas-y0)/h) + 1
	   zbas = 0.25*(zb(nx*(j-1)+i)+zb(nx*(j)+i)+
     *     zb(nx*(j-1)+i+1)+zb(nx*j+i+1));
           k = int((zbas-z0)/h) + 1
	   cell_top=h*(k-1)+z0
...
c          BELOW - TO CALCULATE VEL2
	   ...
	   if (n1.ne.0) then 
	     ...
	     write (*,*) 'Error: vel1 undefined for spn'
     *       ,spn,' at i=',i,' j=',j,' k=',k
	     dtdz = 0
	     goto 650
	   endif
	   if (n2.ne.0) then 
	     ...
	     write (*,*) 'Error: vel2 undefined for spn'
     *       ,spn,' at i=',i,' j=',j,' k=',k
	     dtdz = 0
	     goto 650
	   endif
           dtdz = cosn * ( cos1/vel1 - cos2/vel2 )
650	   write (31,*) nshot_start+qqqq
...
c     Check if number of hw traces O.K.
      if (qqqq.ne.ndive) then
        write (*,*) 'qqqq.ne.ndive at crosspoint file'
        write (*,*) 'qqqq ',qqqq,' ndive ',ndive,' ncrp ',ncrp
	stop
      endif
c
      close (22)
...
c
      ...
      write (*,*) ndive, ' are deep diving wave rays'
c
      open (40,file='last_shot_drw.n',status='unknown')
      write (40,*) nshot_start+ndive
      close (40)
c
 1010 format (a80)
