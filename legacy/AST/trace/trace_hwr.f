...
c
      ...
      parameter    (nxyzmx=20000000,nsmax=50000,nxymax=200000,
     * 		    nshotmax=500)
     ...
     *		   cell_top
      character*80 sfile,tfile,rfile,ufile,pfile,tracefile,zfile,
     *             crossfile,v1file,v2file
c
      write (*,*) 'tracerays2 (based on cover by J.Hole)'
...
c      
      ...
      read (*,*) xs,ys,zs
      write (*,*) xs,ys,zs
      write (*,*) 'input the filename for the ray coverage'
      read (*,1010) rfile
      write (*,*) 'input the filename for the du coverage'
      read (*,1010) ufile
      write (*,*) 'enter 1 if coverage files contain data to which'
      write (*,*) '  the current coverage should be added; else the'
      write (*,*) '  coverage files will be overwritten'
      read (*,*) addcov
      write (*,*) 'input basement depth file'
      read (*,*) zfile
      write (*,*) 'input the filename for the ray traces'
      read (*,*) tracefile
      write (*,*) 'input the filename for the basement crossing points'
      read (*,*) crossfile
...
c     OPEN AND READ (IF REQUIRED) 3D COVERAGE FILES
      if  (addcov .eq. 1)  then
         open (22,file=rfile,form='unformatted',access='direct',
     *        recl=4*(nx-1),status='old')
         open (23,file=ufile,form='unformatted',access='direct',
     *        recl=4*(nx-1),status='old')
         do 20, k=1,nz-1
            nk = (nx-1)*(ny-1)*(k-1)
            nk2 = (k-1)*(ny-1)
            do 15, j=1,ny-1
               nkj = nk+(nx-1)*(j-1)
               read (22,rec=nk2+j) (nrays(nkj+i),i=1,nx-1)
               read (23,rec=nk2+j) (du(nkj+i),i=1,nx-1)
 15         continue
 20      continue
         close (22)
         close (23)
...
c     Open file for basement crossing points
...
c         write (29,1020) line,spn,dt,length
         ...
         ngshot = ngshot + 1
c        We designate head wave ray by 1 and direct by 2
...
c     Uncomment the following line to write out the number of traces
c     at the begining of the file  
c     write (21,*) nbshot
c     This will be used to count HW traces
      qqqq=0
c     ------------------------------------
      do 505, qqq=nshot_start+1,nshot
        write (*,*) 'loop ',qqq
        read (30,*) qq
c     Will save ray only if it is head wave trace
        if (hwshot(qq-nshot_start).eq.1) then
	  qqqq=qqqq+1
	  write (21,*) nshot_start+qqqq
	  write (*,*) qq,' of ',nshot,' numbered as ',nshot_start+qqqq
	else
	  write (*,*) qq, ' of ',nshot, ' is not HW trace'
	endif
...
c     Check if number of hw traces O.K.
      if (qqqq.ne.nbshot) then
        write (*,*) 'qqqq.ne.nbshot'
	stop
      endif
c     ---------------------------------
c     Uncomment the following line to write out the last trace number
c     at the end of the file  
c     write (21,*) nshot_start+nbshot
      close (21)
      close (30)
c
...
c
      ...
      if (iy2d.eq.1) ny=1
c
c     READ SEDIMENT VELOCITY FILE
C     READ CRUSTAL VELOCITY FILE (INTO T)
      open (21,file=v1file,form='unformatted',access='direct',
     *     recl=4*nx,status='old')
      open (23,file=v2file,form='unformatted',access='direct',
     *     recl=4*nx,status='old')
      do 610, k=1,nz
         nk = nx*ny*(k-1)
         nk2 = (k-1)*ny
         do 605, j=1,ny
            ...
            read (23,rec=nk2+j) (t(nkj+i),i=1,nx)
 605     continue
 610  continue
      close (21)
      close (23)
...
c
c     Uncomment the following line to write out the number
c     of crossing points at the beginning of the file
c     write (31,*) nbshot
c
      do 700, m=1,nbshot
c
C        READ RAYS AND BASEMENT INTERSECTION
         read (24,*) nshot
         read (24,1060) line,spn,xbas,ybas,zbas,tt,cos1,cos2,cosn
c
c        CALCULATE DT/DZ AND WRITE
         i = int((xbas-x0)/h) + 1
         j = int((ybas-y0)/h) + 1
	 zbas = 0.25*(zb(nx*(j-1)+i)+zb(nx*(j)+i)+zb(nx*(j-1)+i+1)+zb(nx*j+i+1));
         k = int((zbas-z0)/h) + 1
	 cell_top=h*(k-1)+z0
         iiiii = nx*ny*(k-1)+nx*(j-1)+i
c	 USE CELL VORTICES ABOVE THE INTERFACE TO CALCULATE VEL1 
c        BELOW - TO CALCULATE VEL2
	 vel1=0.0
	 vel2=0.0
	 n1=0
	 n2=0
         if (zb(nx*(j-1)+i).GT.cell_top) then
	   vel1=vel1+v1(iiiii) 
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii) 
	 ...
	 if (zb(nx*(j-1)+i+1).GT.cell_top) then
	   vel1=vel1+v1(iiiii+1)
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii+1)
	   n2=n2+1
	 endif
	 if (zb(nx*(j)+i).GT.cell_top) then
	   vel1=vel1+v1(iiiii+nx)
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii+nx)
	   n2=n2+1
	 endif
	 if (zb(nx*(j)+i+1).GT.cell_top) then
	   vel1=vel1+v1(iiiii+nx+1)
	   n1=n1+1;
	 else
	   vel2=vel2+v1(iiiii+nx+1)
	   n2=n2+1
	 endif
	 if (zb(nx*(j-1)+i).GT.cell_top+h) then
	   vel1=vel1+v1(iiiii+nx*ny)
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii+nx*ny)
	   n2=n2+1
	 endif
	 if (zb(nx*(j-1)+i+1).GT.cell_top+h) then
	   vel1=vel1+v1(iiiii+nx*ny+1)
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii+nx*ny+1)
	   n2=n2+1
	 endif
	 if (zb(nx*(j)+i).GT.cell_top+h) then
	   vel1=vel1+v1(iiiii+nx*ny+nx)
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii+nx*ny+nx)
	   n2=n2+1
	 endif
	 if (zb(nx*(j)+i+1).GT.cell_top+h) then
	   vel1=vel1+v1(iiiii+nx*ny+nx+1)
	   n1=n1+1
	 else
	   vel2=vel2+v1(iiiii+nx*ny+nx+1)
	   n2=n2+1
	 endif
	 if (n1.ne.0) then 
	   vel1=vel1/n1
	 else
	   write (*,*) 'Error: vel1 undefined for spn',spn,' at i=',i,' j=',j,' k=',k
	   dtdz = 0
	   goto 650
	 endif
	 if (n2.ne.0) then 
	   ...
	   write (*,*) 'Error: vel2 undefined for spn',spn,' at i=',i,' j=',j,' k=',k
	   dtdz = 0
	   goto 650
	 endif
         dtdz = cosn * ( cos1/vel1 - cos2/vel2 )
650	 write (31,*) nshot_start+m
         write (31,1030) line,spn,xbas,ybas,zbas,tt,dtdz
c	 NOW THE SECOND CROSSPOINT
         read (24,1060) line,spn,xbas,ybas,zbas,tt,cos1,cos2,cosn
c
c        CALCULATE DT/DZ AND WRITE
         i = int((xbas-x0)/h) + 1
...
c        BELOW - TO CALCULATE VEL2
	 ...
	 if (n2.ne.0) then 
	   ...
	   goto 660
         ...
         dtdz = cosn * ( cos1/vel1 - cos2/vel2 )
 660     write (31,1030) line,spn,xbas,ybas,zbas,tt,dtdz
C
 700  continue
c
      close (22)
