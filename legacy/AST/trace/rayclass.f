      program rayclass
c	
c	Made from tracerays2.f to separate those time peaks that are from
c	head waves from that from direct waves
c	Back-propagates ray till it hits basement. If so writes the time peak
c	to head waves peaks file, otherwise - to direct waves peak file
c	
...
c
     ...
     *             xip,yjp,zkp,ray2(3)
      character*256 sfile,tfile,rfile,ufile,pfile,zfile,
     *             crossfile,v1file,v2file,hwpeakfile,dirpeakfile,
     *		   deeppeakfile,z_next_file
c
...
c      
      ...
      read (*,1010) zfile
      write (*,*) 'input next interface depth file'
      read (*,1010) z_next_file
      write (*,*) 'input the filename for the head wave peaks'
      read (*,1010) hwpeakfile
      write (*,*) 'input the filename for the direct waves peaks'
      read (*,1010) dirpeakfile
      write (*,*) 'input the filename for the deep refracted 
     *             waves peaks'
      read (*,1010) deeppeakfile
c
...
C     OPEN AND READ 2D BASEMENT DEPTH FILE
 ...
 45   continue
      close (22)
C      
C     OPEN AND READ 2D NEXT INTERFACE DEPTH FILE
      open (22,file=z_next_file,form='unformatted',access='direct',
     *         recl=4*nx,status='old')
      do 46, j=1,ny
         nkj = nx*(j-1)
         read (22,rec=j) (zn_b(nkj+i),i=1,nx)
 46   continue
      close (22)     
c
...
c     Open file for deep refracted wave time peaks
      open (32,file=deeppeakfile,status='unknown')
c
c
c
...
c         write (29,1020) line,spn,dt,length
         ...
         ngshot = ngshot + 1
c
c	Write peak to correct file
	if (crossed_flag.eq.0) then
C       DIRECT WAVE PEAK
	 if (pwflag.eq.1) then
	    write (31,*) line,spnp,dist,dum,tpick,ipwght
	 else
	    write (31,*) line,spnp,dist,dum,tpick
	 end if
	 n_dw=n_dw+1
	else
	 if (deep_flag.eq.0) then
C        HEAD WAVE PEAK
	  if (pwflag.eq.1) then
	    write (30,*) line,spnp,dist,dum,tpick,ipwghtc
	  else
	    write (30,*) line,spnp,dist,dum,tpick
	  end if
	  n_hw=n_hw+1
	 else
C        WAVE REFRACTED TOO DEEP	 
	  if (pwflag.eq.1) then
	    write (32,*) line,spnp,dist,dum,tpick,ipwghtc
	  else
	    write (32,*) line,spnp,dist,dum,tpick
	  end if
	  n_qw=n_qw+1
	 end if
        end if
...
c
C     check if ray below the deep interface
      if ((crossed_flag.eq.1).and.(deep_flag.eq.0)) then
        if (z.gt.zn_b(nj+i)) then
          deep_flag=1
        end if
      end if
c
...
c	 if (pwflag.eq.1) then
c	    write (30,*) spnp,dist,dum,tpick,ipwghtc
c	 else
c	    write (30,*) spnp,dist,dum,tpick
c	 end if
c
...
c
      ...
      write (*,*) '         direct wave rays:',n_dw
      write (*,*) '         head wave rays:',n_hw
      write (*,*) '         waves refracted below the next 
     * interface:',n_qw
