...
/*--3D-W19: Use Phii = rii/ndii - ratio of the diagonal to non-diagonal terms-*/
/*--L2 norm as a measure of the resolution power. Empirical measure canged to-*/
/*--normalised summary ray's length inside the support------------------------*/
/*--Parameters and variable names did not change: use W_Nr; p2_Nr; W_Nr_Min_1;*/
/*--float W_Nr_Min_2;, etc. for the analogue values of SumL-------------------*/
/*----------------------------------------------------------------------------*/
/*--08.02.12------------------------------------------------------------------*/
/*--3D-W19-1: Possibility to include apriori information on velocities, i.e.--*/
/*--based on well logs, etc. This information must be arranged as x,y,z,V-----*/
/*--regularisation is applied, regularization parameter needed.---------------*/
/*----------------------------------------------------------------------------*/
/*--24.02.12------------------------------------------------------------------*/
/*--3D-W19-2: Spline interpolation of the UPL velocities on the fine grid-----*/
/*--option included (parameter S, L for linear interpolation)-----------------*/
/*-TODO: 2D case NNx==1 || NNy==1 || NNz==1 are treated but NOT tested--------*/
/*-TODO: SI velocities are still interpolated using linear interpolation only-*/
/*--einspline (http://einspline.sourceforge.net/index.shtml) library must be--*/
/*--installed and -leinspline linker flag used--------------------------------*/
/*----------------------------------------------------------------------------*/
		 ...
		 float z0, float h)
{
  ...
  for (kk=1;kk<=NL;kk++)
  {
   k1=kmin+(kk-1)*kz; if (k1<0) k1=0;
   k2=kmin+kk*kz-1; if (k2>nlay-1) k2=nlay-1;
		  ...
		  float z0, float h)
{
  ...
  if (((N*kav)!=(ncol-1)) || ((M*kav)!=(nrow-1)))
  {
    fprintf(stderr,"N=%ld M=%ld ncol=%ld ncol=%ld kav=%ld\n",N,M,ncol,nrow,kav);
    return -1; /*Error*/
  }
...
int av3d_sub_intf_old(float ***src,float **intf, float **dest,
                  long nlay, long nrow, long ncol, long kav,
		  float z0, float h)
{
  long i, j, k, ii, jj;
...
int bspline_interpolate_UPL
(long Nx, long Ny, long Nz, float h, long NNx, long NNy, long NNz, long uk_min, float dd, float ddz, 
 float const **UL_ft, float const **Zintf, float z0, float const ***UL_dscm, float ****dSFINE, float****VFINE)
{
  long i,j,k,ii,jj,kk;
  float *data; //Buffer for spline construction
  float x,y,z,val;
  Ugrid x_mi, y_mi, z_mi;
//  float UL_z0 = (uk_min-1)*h;
    float UL_z0 = (uk_min)*h;
  
  BCtype_s   x_BC, y_BC, z_BC;   //Boundary conditions.
  x_BC.lCode = y_BC.lCode = z_BC.lCode = NATURAL;
  x_BC.rCode = y_BC.rCode = z_BC.rCode = NATURAL;
  
  
  if ((NNz>1) && (NNx >1) && (NNy>1))
  {
    data = (float*)fmemalloc((size_t)NNx*NNy*NNz,sizeof(float));
    fprintf(stderr,"Data bufferisation...");
    for (ii=1;ii<=NNx;ii++)
      for (jj=1;jj<=NNy;jj++)
        for (kk=1;kk<=NNz;kk++)
        {
	  //fprintf(stderr,"%ld %ld %ld %ld\n",ii,jj,kk,(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1));
	  data[(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1)]=UL_dscm[kk][jj][ii];
        }
    fprintf(stderr,"Success\n");
    x_mi.num=NNx; x_mi.start=0.0-0.1*h; x_mi.end=x_mi.start+(x_mi.num)*dd+0.2*h;
    y_mi.num=NNy; y_mi.start=0.0-0.1*h; y_mi.end=y_mi.start+(y_mi.num)*dd+0.2*h;
    z_mi.num=NNz; z_mi.start=UL_z0-0.1*h; z_mi.end=z_mi.start+(z_mi.num)*ddz+0.2*h;
    fprintf(stderr,"x_mi: %g-%g %ld y_mi: %g-%g %ld z_mi: %g-%g %ld\n",x_mi.start,x_mi.end,x_mi.num,y_mi.start,y_mi.end,y_mi.num,z_mi.start,z_mi.end,z_mi.num);
  }
  else
  {
    if (NNz<1 || NNx<1 || NNy<1) errquit("Zero dimension encontered in spline interpolation\n");
    if (NNz==1)
    {
      fprintf(stderr,"2D model in XY plane encountered");
      if (NNx==1 || NNy==1) errquit("Two dimensions of the inversion grid equal to 1. Can not interpolate 1D models.\n");
      NNz++;
      data = (float*)fmemalloc((size_t)NNx*NNy*NNz,sizeof(float));
      fprintf(stderr,"Data bufferisation...");
      for (ii=1;ii<=NNx;ii++)
        for (jj=1;jj<=NNy;jj++)
          ...
          {
	  //fprintf(stderr,"%ld %ld %ld %ld\n",ii,jj,kk,(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1));
	    data[(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1)]=UL_dscm[1][jj][ii];
          }
      fprintf(stderr,"Success\n");
      x_mi.num=NNx; x_mi.start=0.0-0.1*h; x_mi.end=x_mi.start+(x_mi.num)*dd+0.2*h;
      y_mi.num=NNy; y_mi.start=0.0-0.1*h; y_mi.end=y_mi.start+(y_mi.num)*dd+0.2*h;
      z_mi.num=NNz; z_mi.start=UL_z0-0.1*h; z_mi.end=z_mi.start+(1)*ddz+0.2*h;
      fprintf(stderr,"x_mi: %g-%g %ld y_mi: %g-%g %ld z_mi: %g-%g %ld\n",x_mi.start,x_mi.end,x_mi.num,y_mi.start,y_mi.end,y_mi.num,z_mi.start,z_mi.end,z_mi.num);
    }
    if (NNx==1)
    {
      fprintf(stderr,"2D model in YZ plane encountered");
      if (NNy==1 || NNz==1) errquit("Two dimensions of the inversion grid equal to 1. Can not interpolate 1D models.\n");
      NNx++;
      data = (float*)fmemalloc((size_t)NNx*NNy*NNz,sizeof(float));
      fprintf(stderr,"Data bufferisation...");
      for (ii=1;ii<=NNx;ii++)
        for (jj=1;jj<=NNy;jj++)
          ...
          {
	  //fprintf(stderr,"%ld %ld %ld %ld\n",ii,jj,kk,(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1));
	    data[(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1)]=UL_dscm[kk][jj][1];
          }
      fprintf(stderr,"Success\n");
      x_mi.num=NNx; x_mi.start=0.0-0.1*h; x_mi.end=x_mi.start+(1)*dd+0.2*h;
      y_mi.num=NNy; y_mi.start=0.0-0.1*h; y_mi.end=y_mi.start+(y_mi.num)*dd+0.2*h;
      z_mi.num=NNz; z_mi.start=UL_z0-0.1*h; z_mi.end=z_mi.start+(z_mi.num)*ddz+0.2*h;
      fprintf(stderr,"x_mi: %g-%g %ld y_mi: %g-%g %ld z_mi: %g-%g %ld\n",x_mi.start,x_mi.end,x_mi.num,y_mi.start,y_mi.end,y_mi.num,z_mi.start,z_mi.end,z_mi.num);
    }
    if (NNy==1)
    {
      fprintf(stderr,"2D model in XZ plane encountered");
      if (NNx==1 || NNz==1) errquit("Two dimensions of the inversion grid equal to 1. Can not interpolate 1D models.\n");
      NNy++;
      data = (float*)fmemalloc((size_t)NNx*NNy*NNz,sizeof(float));
      fprintf(stderr,"Data bufferisation...");
      for (ii=1;ii<=NNx;ii++)
        for (jj=1;jj<=NNy;jj++)
          ...
          {
	  //fprintf(stderr,"%ld %ld %ld %ld\n",ii,jj,kk,(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1));
	    data[(ii-1)*(NNy*NNz)+(jj-1)*(NNz)+(kk-1)]=UL_dscm[kk][1][ii];
          }
      fprintf(stderr,"Success\n");
      x_mi.num=NNx; x_mi.start=0.0-0.1*h; x_mi.end=x_mi.start+(x_mi.num)*dd+0.2*h;
      y_mi.num=NNy; y_mi.start=0.0-0.1*h; y_mi.end=y_mi.start+(1)*dd+0.2*h;
      z_mi.num=NNz; z_mi.start=UL_z0-0.1*h; z_mi.end=z_mi.start+(z_mi.num)*ddz+0.2*h;
      fprintf(stderr,"x_mi: %g-%g %ld y_mi: %g-%g %ld z_mi: %g-%g %ld\n",x_mi.start,x_mi.end,x_mi.num,y_mi.start,y_mi.end,y_mi.num,z_mi.start,z_mi.end,z_mi.num);
    }
  }
  //x_mi.num=NNx; x_mi.start=0.5*dd; x_mi.end=x_mi.start+(x_mi.num-1)*dd;
  //y_mi.num=NNy; y_mi.start=0.5*dd; y_mi.end=y_mi.start+(y_mi.num-1)*dd;
  //z_mi.num=NNz; z_mi.start=UL_z0+0.5*ddz; z_mi.end=z_mi.start+(z_mi.num-1)*ddz;
  
  fprintf(stderr,"Spline creation...");
  UBspline_3d_s *Spline    = create_UBspline_3d_s( x_mi, y_mi, z_mi, x_BC, y_BC, z_BC, data);
  fprintf(stderr,"Success\n");      
  fmemfree(data);
  
  fprintf(stderr,"Spline evaluation...");
  for (k=0;k<Nz;k++)
  {
    z=h*(float)k;
    for (j=0;j<Ny;j++)
	...
	if ((k>=floor((UL_ft[j][i]-z0)/h)) && (k<floor((Zintf[j][i]-z0)/h))) /*Node inside reference UPL*/
	{
	  ...
	  //fprintf(stderr," %g\n",val);
	  (*dSFINE)[k][j][i]=val;
	  (*VFINE)[k][j][i]=(*VFINE)[k][j][i]/(1.0+(*VFINE)[k][j][i]*val);
...
int cilk_main(int narg, char **argv)
{
 ...
 // float ***URL;		/*Reflected wave rays length's in the cells above the interface*/
  ...
  char interpolate='L';	//Type of interpolation to fine grid 'L'=='linear', 'S'==spline
  ...
  /*--W19-1: apriori velocity stuff--*/
  float ***s0;   //reference model slowness
  apriori_slowness temp_sa;  //for push_back();
  std::vector<apriori_slowness> S_apriori;  //apriori UPL velocity information
  float w_sa;  //apriori slowness regularisation parameter
  long Ns;  //Number of inversion cells with the apriori slowness info
  
  char sa_filename[STRLN];	//apriori velocity values filename
  
  long Nv;  //number of apriori velocity info points (x,y,z,V)
  apriori_velocity_info *V_apriori;  //apriori velocity original information (x,y,z,V)
  
  long found;
  /*--W19-1: END of apriori velocity stuff--*/
  ...
  if (fscanf(fp,"%g%g%g",&lambda_x,&lambda_y,&lambda_z)!=3) errquit("Error:Please enter 3 Rougness regularization parameters 2 for horizontal and 1 vertical directions");
  ...
  fscanf(fp,"%s%g",sa_filename,&w_sa);   fgets(buf,STRLN,fp);  /*9th LINE: Velocity apriori info data file and weighting factor*/
  fscanf(fp,"%s%g",fiWells,&ww);   fgets(buf,STRLN,fp);  /*10th LINE: Wells (reflection) data file and
  							   general weighting factor*/
  fscanf(fp,"%g",&W_Nr_Min_1); fscanf(fp,"%g",&W_Nr_Min_2);
  fgets(buf,STRLN,fp);  	/*11th LINE: Minimum numbers 1 and 2 of ray length per wavelet support
                                                     to include it into inversion*/
  fscanf(fp,"%g",&p2_Nr); fgets(buf,STRLN,fp);	/*12th LINE: Number of ray length per wavelet support is nomalised by
  						      2^(p2_Np*l), l - wavelet order*/
  fscanf(fp,"%g",&W_Np_Min); fgets(buf,STRLN,fp);   /*13th LINE: Minimum number of refraction+reflection
  						      points per wavelet support*/
  fscanf(fp,"%g",&p2_Np); fgets(buf,STRLN,fp);	/*14th LINE: Number of points per wavelet support is nomalised by
  						      2^(p2_Np*l), l - wavelet order*/
  fscanf(fp,"%d%d",&use_down_hr,&use_up_hr);
  fgets(buf,STRLN,fp);				    /*15th LINE: Flags to use (1) or not (0) downgoing
                                                                 and upgoing headwave rays*/
  fscanf(fp,"%d%d",&use_down_rr,&use_up_rr);
  fgets(buf,STRLN,fp);				    /*16st LINE: Flags to use (1) or not (0) downgoing
                                                                 and upgoing reflected rays*/
  fscanf(fp,"%s",fi_VelMod);	fgets(buf,STRLN,fp); /*17th LINE: Fine true reference velocity model*/
  fscanf(fp,"%s",fi_fiBas);	fgets(buf,STRLN,fp); /*18th LINE: Fine reference interface position*/
  fscanf(fp,"%s",fi_fiTop);	fgets(buf,STRLN,fp); /*19th LINE: Fine layer top position*/
  fscanf(fp,"%d",&Sm_target);	fgets(buf,STRLN,fp); /*20th LINE: Smooth model flag (1 -target, 0 - variance)*/
  fscanf(fp,"%g",&Theta_1); fscanf(fp,"%g",&Theta_2);
  fgets(buf,STRLN,fp);		 /*21st LINE: Minimum angular deviation values 1 and 2 (radian)*/
  fscanf(fp,"%g",&Theta_UPL_1); fscanf(fp,"%g",&Theta_UPL_2);
  fgets(buf,STRLN,fp);		/*22nd LINE: Minimum angular deviation values 1 and 2 (radian) for UPL*/
  fscanf(fp,"%g",&p2_Theta); fgets(buf,STRLN,fp);	/*23rd LINE: angular deviation is nomalised by
  						      2^(p2_Theta*l), l - wavelet order*/
  fscanf(fp,"%g%g",&Rsmooth,&Psmooth); fgets(buf,STRLN,fp); /*24th LINE: Radius and power for additional
  								smoothing of interface*/
  fscanf(fp,"%g%g",&Rsmooth_ds,&Psmooth_ds); fgets(buf,STRLN,fp); /*25th LINE: Radius and power for additional
  								smoothing of sub-interface slowness variation*/
  fscanf(fp,"%d",&scalefunc_include); fgets(buf,STRLN,fp); /*26th LINE: Include (1) or not (0) the scaling function,
  								i.e. constant in all expansions*/
  fscanf(fp,"%d",&estimate_resolution); fgets(buf,STRLN,fp); /*27th LINE: Estimate (1) or not (0) the resolution
  								ma trix: time consuming!*/
  fscanf(fp,"%d",&write_si_pattern); fgets(buf,STRLN,fp);	/*28th LINE Write to file or not the subinterface wavelet coverage pattern*/
  fscanf(fp,"%c",&interpolate); fgets(buf,STRLN,fp);	/*29th LINE Write to file or not the subinterface wavelet coverage pattern*/
  if (fscanf(fp,"%s",output_path)!=1) output_path[0]='\0'; fgets(buf,STRLN,fp);	/*30th LINE: Path for output files*/
  ...
  fprintf(stderr,"Interpolation type: %c\n",interpolate);
 ...
 if (eq_digital((Tmin-z0)/h,roun((Tmin-z0)/h),1e-7*h)) {fprintf(stderr,"%g %g ",(Tmin-z0)/h,roun((Tmin-z0)/h)); uk_min=roun((Tmin-z0)/h)+1;}
 else uk_min=floor((Tmin-z0)/h)+1; fprintf(stderr,"uk_min=%ld\n",uk_min);
 if (eq_digital((Zmax-z0)/h,roun((Zmax-z0)/h),1e-7*h)) {fprintf(stderr,"%g %g ",(Zmax-z0)/h,roun((Zmax-z0)/h)); uk_max=roun((Zmax-z0)/h);}
  ...
  */
  ...
  s0=f3tensor(1,NNz,1,NNy,1,NNx);
  UL_dscm=f3tensor(1,NNz,1,NNy,1,NNx);

  fprintf(stderr,"check point 1\n");
  
  
  fprintf(stderr,"%lu %ld %ld %ld %ld %ld %ld %g %g\n",Nz,Ny,Nx,k_gen,uk_min,k_gen_v,NNz,z0,h);
  av3d_ins_lay(VFINE,UL_ft,Zintf,s0,Nz,Ny,Nx,k_gen,uk_min,k_gen_v,NNz,z0,h);
  ...
  if (Sm_target==1)
  {
	...
	for (kk=1;kk<=NNz;kk++)
	{
	  if (s0[kk][jj][ii]<=0) errquit("Zero or negative UPL velocity encountered");
	  //NEEDED FOR DEBUG PURPOSES
	  /*
	  if ((s0[kk][jj][ii]>=3.0) && (s0[kk][jj][ii]<DUMMY_LIM))
	  {
	    fprintf(stderr,"%ld %ld %ld %g\n",kk,jj,ii,s0[kk][jj][ii]);
	  }
	  */
	  //-------------------------
	  if (s0[kk][jj][ii] < DUMMY_LIM) UL_dscm[kk][jj][ii]=1.0/s0[kk][jj][ii];
	}
      }
  }
  else
...
/*
  ...
  for (m=1;m<=Nl;m++)
  {
    ...
    while (ncell!=-1)
    {
      ...
      if (((raypos==0) || (raypos==1)) && ((kk<1) || (kk>NNz)))
      {
//         fprintf(stderr,"WARNING: ray %ld with raypos %d out of layer model in cell %d\n",
// 			nray,raypos,ncell);
      }    
      ...
      switch (raypos)
      {
       	case 0: if (use_down_rr && ll> 0) if ((kk>=1) && (kk<=NNz)) //верхний солой
       	        {
       		...
       		break;
	case 1: if (use_up_rr && ll>0) if ((kk>=1) && (kk<=NNz)) //верхний солой
		{
		  it=Cells_reflected[kk-1][bigcelln-1].find(nray);
                  if( it!=Cells_reflected[kk-1][bigcelln-1].end())
                    {
  ...
  for (kk=1;kk<=NNz;kk++)
  {
     ...
     */
     ...
     for (wl=GL[kk].WI_J;wl>=1;wl--)
	  ...
	  /*W19: here changed W_Nr to SumL, i.e. suppose that the thresholds values are given accordingly*/
	  if (treppe_crit(W_Nr_Min_1,W_Nr_Min_2,Theta_UPL_1,Theta_UPL_2,
                          (float)SumL/(float)l2n(p2_Nr*wl),(float)z/(float)l2n(p2_Theta*wl))) {
	    GL[kk].w_N++;
...
/*
  ...
  */
  ...
  for (wl=W_J;wl>=1;wl--)
  {
	...
	/*W19: here changed W_Nr to SumL, i.e. suppose that the thresholds values are given accordingly*/
	if (treppe_crit(W_Nr_Min_1,W_Nr_Min_2,Theta_1,Theta_2,(float)SumL/(float)l2n(p2_Nr*wl),
  ...
  fi=file_open(sa_filename,"rt");
  fscanf(fi,"%ld",&Nv);
  V_apriori = (apriori_velocity_info*)fmemalloc(Nv,sizeof(apriori_velocity_info));
  fprintf(stderr,"%ld velocity values will be read...\n",Nv);
  for (i=0;i<Nv;i++)
  {
    fscanf(fi,"%g%g%g%g",&(V_apriori[i].x),&(V_apriori[i].y),&(V_apriori[i].z),&(V_apriori[i].v));
  }
  fclose(fi);
  fprintf(stderr,"Success.\n");
  
  /*--Averaging apriori velocity info inside inversion cells--*/
//   fprintf(stderr,"NNx=%d NNy=%d NNZ=%d .\n",NNx,NNy,NNz);
  for (i=0;i<Nv;i++)
  {
    kk = floor((V_apriori[i].z-UL_H_min)/dd)+1;
    ii = floor((V_apriori[i].x-x0)/dd)+1;
    jj = floor((V_apriori[i].y-y0)/dd)+1;
//     fprintf(stderr,"Point kk=%d ii=%d jj=%d .\n",kk,ii,jj);
    if (kk>=1 && kk<=NNz && ii>=1 && ii<=NNx && jj>=1 && jj<=NNy)
    {
      //fprintf(stderr,"i=%d of %d %d\n",i,Nv,S_apriori.size());
      found=0;
      for (j=0;j<(int)S_apriori.size();j++)
      {
        if (S_apriori[j].ii==ii && S_apriori[j].jj==jj && S_apriori[j].kk==kk)
        {
	  //fprintf(stderr,"Found: %d\n",j);
	  S_apriori[j].sa+=V_apriori[i].v;
	  S_apriori[j].n++;
	  found=1;
	  //fprintf(stderr,"Success\n");
        }
      ...
      if (!found)
      {
        ...
        temp_sa.jj=jj;
        temp_sa.ii=ii;
        temp_sa.sa=V_apriori[i].v;
        temp_sa.n=1;
        S_apriori.push_back(temp_sa);
        //fprintf(stderr,"Success\n");
      }
    }
    else
      fprintf(stderr,"Point X=%g Y=%g Z=%g is out of UPL. Ignored.\n",V_apriori[i].x,V_apriori[i].y,V_apriori[i].z);
  }  //END: for i = 0,..,Ns-1
  //fprintf(stderr,"Ns=%d\n",S_apriori.size());
  for (j=0;j<(int)S_apriori.size();j++)
  {
    ...
    jj = S_apriori[j].jj;
    ii = S_apriori[j].kk;
    /*fprintf(stderr,"S apriori: kk=%ld jj=%ld ii=%ld V=%g s=%g s0=%g\n",
				S_apriori[j].kk,S_apriori[j].jj,S_apriori[j].ii,
				S_apriori[j].sa/S_apriori[j].n,S_apriori[j].n/S_apriori[j].sa,1.0/s0[kk][jj][ii]);*/
    S_apriori[j].sa=S_apriori[j].n/S_apriori[j].sa-1.0/s0[kk][jj][ii];
  }
  Ns = S_apriori.size();  //Number of inversion cells with the apriori slowness info
  fprintf(stderr,"Number of apriori slowness information regularization equations %ld\n",Ns);
  
  free_f3tensor(s0,1,NNz,1,NNy,1,NNx);
  fmemfree(V_apriori);
  /*--S_apriori[j].sa now contains the desired correction to slowness--*/
  /*--END: Averaging apriori velocity-------------------------*/
  ...
  /*+ Nw interface apriori information equations*/
  /*+ Ns slowness apriori information equations*/
  /*+ 2*N ds smoothing equations*/
  /*+ 2*N dz smoothing equations*/
  Num_E=Nl+Nr+Nw+Ns+2*N+2*N;
...
//free_vector(ss_r,1,Nr);
  ...
  N0+=Nr; /*3 block row*/
  CB.Vbr[3]=N0+1;
  fprintf(stderr,"Now forming BLOCK VI (dZ a priory)...\n");// не меняю
  c0=w_s1_N;
  for (r=1;r<=Nw;r++)
  {
    i = r;
  ...
  N0+=Nw; /*4 block row*/
  CB.Vbr[4]=N0+1;
  fprintf(stderr,"Now forming a priory UPL slowness equations...\n");// не меняю
  c0=w_s1_N+w_z1_N;
  for (r=0;r<Ns;r++)
  {
    i = r;
    c0=w_s1_N+w_z1_N;
    for (kk=1;kk<S_apriori[i].kk;kk++) c0+=GL[kk].w_ncf;
    
    kk=S_apriori[i].kk;
    ii=S_apriori[i].ii;
    jj=S_apriori[i].jj;
    
    //fprintf(stderr,"Cell: %ld %ld %ld\n",kk,jj,ii);
    
    for (c=1;c<=GL[kk].w_ncf;c++)
    {
      value.push_back(w_sa*ev_haar(c,GL[kk].WV,GL[kk].W_Si+ii,GL[kk].W_Sj+jj));
      row.push_back(N0+r);
      column.push_back(c0+c);   
      matrix_count++;    
      //fprintf(stderr,"Push: %ld %ld %ld %g\n",c,N0+r,c0+c,w_sa*ev_haar(c,GL[kk].WV,GL[kk].W_Si+ii,GL[kk].W_Sj+jj));
    }
  }
  ...
  std::vector<double> *sm_vec = new vector<double>(W_Ni*W_Ni,0.0); //??????????? N
  c0=0;
  for (r=1;r<=N;r++)
  {
  ...
  for (kk=1;kk<=NNz;kk++)
  {
    ...
    //fprintf(stderr,"jj_min=%ld jj_max=%ld ii_min=%ld ii_max=%ld N=%ld w_ncf=%ld\n",GL[kk].jj_min,GL[kk].jj_max,GL[kk].ii_min,GL[kk].ii_max,N,GL[kk].w_ncf);
    //fprintf(stderr,"******************************************************************************************\n");
    ...
    for (r=1;r<=GL[kk].NC;r++)
    {
      ...
      //fprintf(stderr,"r=%ld jj=%ld ii=%ld\n",r,jj,ii);
      if ((ii>=GL[kk].ii_min+1) && (ii<=GL[kk].ii_max-1))
      {
//         fprintf(stderr,"Smoothing X\n");
...
// 	fprintf(stderr,"++++++++++++%ld %ld\n",kk,GL[kk].jj_max);
  ...
  N0+=Nr; /*3 block row*/
  fprintf(stderr,"Now forming BLOCK III (Za-Zref)\n");
  for (r=1;r<=Nw;r++)
  {
    i = r;
    ii = floor((wx[i]-x0)/dd)+1;
  ...
  N0+=Nw; /*4 block row*/
  fprintf(stderr,"Now forming BLOCK IV (Sa-Sref)\n");
  for (r=1;r<=Ns;r++)
  {
    i = r - 1;
    B[N0+r]=w_sa*S_apriori[i].sa;
  }
  fprintf(stderr,"Success.\n");

  N0+=Ns; /*5 block row*/
  fprintf(stderr,"Now forming BLOCK V (- Sm X ds)\n");
  for (r=1;r<=N;r++)
  {
    pos_from_cln(&jj,&ii,r,NNx);
    if ((ii>=2) && (ii<=NNx-1))
  ...
  N0+=N; /*6 block row*/
  fprintf(stderr,"Now forming BLOCK VI (- Sm Y ds)\n");
  for (r=1;r<=N;r++)
  {
    pos_from_cln(&jj,&ii,r,NNx);
//     fprintf(stderr,"r=%ld jj=%ld ii=%ld\n",r,jj,ii);
  ...
  N0+=N; /*7 block row*/
  fprintf(stderr,"Now forming BLOCK VII (- Sm X dZ0)\n");
  for (r=1;r<=N;r++)
  {
    pos_from_cln(&jj,&ii,r,NNx);
    if ((ii>=2) && (ii<=NNx-1))
  ...
  N0+=N; /*8 block row*/
  fprintf(stderr,"Now forming BLOCK VIII (- Sm Y dZ0)\n");
  for (r=1;r<=N;r++)
  {
    pos_from_cln(&jj,&ii,r,NNx);
    if ((jj>=2) && (jj<=NNy-1))
  ...
  for (kk=1;kk<=NNz;kk++)
  {
    for (r=1;r<=GL[kk].NC;r++)
    {
      ...
      if ((ii>=GL[kk].ii_min+1) && (ii<=GL[kk].ii_max-1))
      {
// 	fprintf(stderr,"Smoothing X RHS\n");
        if ((UL_dscm[kk][jj][ii] < DUMMY_LIM) &&
	    (UL_dscm[kk][jj][ii-1] < DUMMY_LIM) &&
	    (UL_dscm[kk][jj][ii+1] < DUMMY_LIM))
    ...
    for (r=1;r<=GL[kk].NC;r++)
    {
      ...
      if ((jj>=GL[kk].jj_min+1) && (jj<=GL[kk].jj_max-1))
      {
//         fprintf(stderr,"Smoothing Y RHS\n");
	if ((UL_dscm[kk][jj][ii] < DUMMY_LIM) &&
	    (UL_dscm[kk][jj-1][ii] < DUMMY_LIM) &&
	    (UL_dscm[kk][jj+1][ii] < DUMMY_LIM))
...
// 	fprintf(stderr,"++++++++++++%ld %ld\n",kk,GL[kk].jj_max);
  ...
  */
  ...
  float ndi;	/*Non-diagonal R elements norm*/
  ...
  if (estimate_resolution)
  {
   ...
   for (i=1;i<=w_s1_N;i++)
   {
     ...
     ndi=sqrt(((REdiff[N0+i-1]-pow((double)RM[N0+i-1][N0+i-1]-1,(double)2.0)))/(Num_U-2));
     fprintf(fo_resol,"%ld %ld %ld %ld %ld %d %g %g %g %g %g %g %g\n"
     		     ,N0+i,wl,wm,wn,wk,W_Nr,y,z,sqrt(REdiff[N0+i-1]/(Num_U-1)),RM[N0+i-1][N0+i-1],ndi,RM[N0+i-1][N0+i-1]/ndi,SumL);
   }
   file_close(fo_resol);
   file_close(fi);
   N0+=w_s1_N;
   ...
   for (i=1;i<=w_z1_N;i++)
   {
     ...
     fprintf(fo_resol,"%d %ld %ld %ld %ld %ld %ld %g %g %g\n"
     		     ,wq,wl,wm,wn,wk,W_Np_h,W_Np_r,z,REdiff[N0+i-1],RM[N0+i-1][N0+i-1]);
   }
   file_close(fo_resol);
   file_close(fi);
   N0+=w_z1_N;
  ...
  for (j=0;j<Ny;j++)
  {
	...
	{
         SI_dsv_fine[j][i]=(1.0/(dd*dd))*
	  (SI_dscm[jj+1][ii]*(dd-x)*(dd-y)+
	   SI_dscm[jj+1][ii+1]*x*(dd-y)+
	   SI_dscm[jj+1][ii]*(dd-x)*y+
	   SI_dscm[jj+1][ii+1]*x*y);
	}
	else
	{
	  SI_dsv_fine[j][i]=(1.0/(dd*dd))*
	  (SI_dscm[jj][ii+1]*(dd-x)*(dd-y)+
	   SI_dscm[jj][ii+1]*x*(dd-y)+
	   SI_dscm[jj+1][ii+1]*(dd-x)*y+
	   SI_dscm[jj+1][ii+1]*x*y);
	}
      }
    }
  ...
  if (interpolate=='L')
  {
   fprintf(stderr,"Type of interpolation: linear\n");
   if (NNz!=1)
   {
   for (k=0;k<Nz;k++)
   {
    ...
    //z=h*(float)(k-uk_min)-((float)kk-0.5)*ddz;
    if (NNx!=1 && NNy!=1)
    {
     for (j=0;j<Ny;j++)
     {
	...
	for (j=0;j<Ny;j++)
	    ...
	    if ((k>floor((UL_ft[j][i]-z0)/h)) && (k<floor((Zintf[j][i]-z0)/h))) /*Node inside reference UPL*/
	    {
	      ...
	      ss=(1.0/(dd*ddz))*
	       (UL_dscm[kk][jj][ii]*(ddz-z)*(dd-x)+
	        UL_dscm[kk][jj][ii+1]*x*(ddz-z)+
	        UL_dscm[kk+1][jj][ii]*(dd-x)*z+
	        UL_dscm[kk+1][jj][ii+1]*x*z);
	...
	for (j=0;j<Ny;j++)
           ...
           if ((k>floor((UL_ft[j][i]-z0)/h)) && (k<floor((Zintf[j][i]-z0)/h))) /*Node inside reference UPL*/
	    {
	       ...
	       ss=(1.0/(dd*ddz))*
	       (UL_dscm[kk][jj][ii]*(ddz-x)*(dd-y)+
	        UL_dscm[kk][jj+1][ii]*y*(ddz-z)+
	        UL_dscm[kk+1][jj][ii]*(dd-y)*z+
	        UL_dscm[kk+1][jj+1][ii]*y*z);
  ...
  } // END linear interpolation case
  else
  {
    fprintf(stderr,"Type of interpolation: spline\n");
    bspline_interpolate_UPL(Nx,Ny,Nz,h,NNx,NNy,NNz,uk_min,dd,ddz,(const float**)UL_ft,(const float**)Zintf,z0,(const float***)UL_dscm,&dSFINE,&VFINE); 
  }
  fprintf(stderr,"Success.\n");
  ...
  for (j=0;j<Ny;j++)
  {
	...
	{
	 ss=(1.0/(dd*dd))*
	  (I_dzcm[jj+1][ii]*(dd-x)*(dd-y)+
	   I_dzcm[jj+1][ii+1]*x*(dd-y)+
	   I_dzcm[jj+1][ii]*(dd-x)*y+
	   I_dzcm[jj+1][ii+1]*x*y);
	}
	else
	{
	  ss=(1.0/(dd*dd))*
	  (I_dzcm[jj][ii+1]*(dd-x)*(dd-y)+
	   I_dzcm[jj][ii+1]*x*(dd-y)+
	   I_dzcm[jj+1][ii+1]*(dd-x)*y+
	   I_dzcm[jj+1][ii+1]*x*y);
	}
      }
