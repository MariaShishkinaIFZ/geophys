/*
* lsqr.c
* This is a CPP version of LSQR, derived from the Fortran 77 implementation
...
* 27 Feb 2006: Modified by S.Tikhotsky <sat@ifz.ru>
*/
	   ...
	   void *prod )
{
  ...
  static char term_msg[10][80] = 
  {
    ...
    "The residual norm ||Ax - b|| is less than machine precision: exact solution",
    "The residual norm ||Ax - b|| is less than given rms data error"
  };
...
*/
      ...
      res += sqr(psi);
      output->resid_norm = sqrt( sqr(phibar) + res ); /*||rbar||*/
...
*/
	...
	output->term_flag = 4;
	/*||Ax - b|| less than machine precision */
      if( output->resid_norm <= DBL_EPSILON)
        output->term_flag = 8;
	...
	output->term_flag = 1;
	/*||Ax - b|| less than data rms error */
      if( output->resid_norm <= input->abs_resid)
        output->term_flag = 9;
/*
*  If statistics are printed at each iteration, print a header and the initial
*  values for each quantity.
