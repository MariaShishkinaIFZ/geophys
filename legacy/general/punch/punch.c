...
****Hole, J.A., and B.C. Zelt, 1995.  3-D finite-difference reflection 
    ...
    edited by S.Tikhotsky 12.04.04 to fix possible running of the expanding process
    	outside the TIMEFIELD area at the borders
*/
	...
	if(dz1){
		...
		for(i=0;i<ii;i++){
			...
			if(time0[index-1] < 1.e9 && time0[index-nx-1] < 1.e9
			  ...
			  if (X2<ny-1 && X1>x1+1 && X1<x2-1)
			  if(time0[index+nx] < 1.e9)  {
