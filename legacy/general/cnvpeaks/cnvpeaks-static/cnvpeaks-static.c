...
int main(int narg, char **argv)
{
  ...
  if (narg<8)
  {
    printf("Insufficient parameters.\nUSAGE: cnvpeaks <peaks data file> <out file> xs ys zs Z_static V_static \n");
    exit(1);
  }
  ...
  sscanf(argv[4],"%g",&ys);
  sscanf(argv[5],"%g",&zs);
  sscanf(argv[6],"%g",&Z_static);
  sscanf(argv[7],"%g",&V_static);
  ...
  for (c=0;c<Np;c++)
  {
    ...
    tp=tp+(zp-zs)/V_static+(zs-zs)/V_static;
