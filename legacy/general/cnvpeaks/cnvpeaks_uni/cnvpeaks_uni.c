...
int main(int narg, char **argv)
{
  ...
  if (narg<4)
  {
    printf("Insufficient parameters.\nUSAGE: cnvpeaks <reciver's prof ID  X Y Z file> <source's ID  X Y Z file> <peaks data file 5 columns>  <OUT peaks.tpk_file>\n");
    exit(1);
  }
...
//   sscanf(argv[4],"%s",&s_id);
  ...
  while (!feof(fi))
  {
    sscanf(s,"%d%s%d%d%g",&prof_s,&id_s,&prof_p,&id_p,&tp);
...
// 	  printf("%d %d \n",id_p,id_tmp);
    ...
    if (found) {fprintf(fo,"%3d  %6s       %5d  %5d        %10g %5g       %10.6g\n",prof_s,id_s,prof_p,id_p,d,az,tp); c++; }
    fgets(s,STRLN,fi);
  }
  printf("%d peaks done.\n",c);
