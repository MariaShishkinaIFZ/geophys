...
int main(int narg, char **argv)
{
  ...
  if (ask) fprintf(stdout,"Please enter alpha (XZ plane rotation angle, degrees)  > \n");
  fscanf(fp,"%g",&a);
  a*=M_PI/180.0;
  ...
  for (k=0;k<Ny;k++)
    for (i=0;i<Nz;i++)
      for (j=0;j<Nx;j++)
      {
        xrot=(x0+h*j)*cos(a)+(z0+h*i)*-sin(a);
	zrot=(x0+h*j)*sin(a)+(z0+h*i)*cos(a);
	V[i][k][j]=V[i][k][j]*
                    (1+rel_anom*ch1d(y0+h*k,cy0,cyw,cyt)*ch1d(zrot,cz0,czw,czt)*ch1d(xrot,cx0,cxw,cxt));
      }
