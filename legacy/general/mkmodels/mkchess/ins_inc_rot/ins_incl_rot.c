...
int main(int narg, char **argv)
{
  ...
  if (ask) fprintf(stdout,"Please enter alpha (XY plane rotation angle, degrees)  > \n");
  fscanf(fp,"%g",&al);
  al*=M_PI/180.0;
  ...
  for (k=0;k<Nz;k++)
    for (i=0;i<Ny;i++)
      for (j=0;j<Nx;j++)
      {
	xrot=(x0+h*j)*cos(al)+(y0+h*i)*-sin(al);
	yrot=(x0+h*j)*sin(al)+(y0+h*i)*cos(al);
        incl_Z=ch1d(z0+h*k,cz0,czw,czt);
        incl_Y_rot=ch1d(yrot,cy0_rot,cyw,cyt);
        incl_X_rot=ch1d(xrot,cx0_rot,cxw,cxt);
	
        if (incl_X_rot<EMPTY_LIMIT && incl_Y_rot<EMPTY_LIMIT && incl_Z<EMPTY_LIMIT)
        V[k][i][j]+=(abs_vel-V[k][i][j])*incl_X_rot*incl_Y_rot*incl_Z;
      }
