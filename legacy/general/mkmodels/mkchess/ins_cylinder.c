...
float ch1d(float x0, float y0, float z0, float R, float t,float x, float y, float z)
{
  ...
  {
    if (r<=R) return a;
    else return a*sin(M_PI*(r-R)/(2.0*t));
  }
  else
    return EMPTY_VALUE;
}
...
int main(int narg, char **argv)
{
  ...
  fscanf(fp,"%c",&cs);
	...
	fscanf(fp,"%g%g%g%g",&cx0,&cz0,&cR,&ct); cy0=0;
      case 'Z': 
	if (ask) fprintf(stdout,"Please enter x0, yO, R and t - wide of transit  > \n");
	fscanf(fp,"%g%g%g%g",&cx0,&cy0,&cR,&ct); cz0=0;
  ...
  for (k=0;k<Nz;k++)
	...
	switch (cs)
	{
	 case 'X': incl_C=ch1d(x0+h*j,cy0,cz0,cR,ct,x0+h*j,y0+h*i,z0+h*k);
	 case 'Y': incl_C=ch1d(cx0,y0+h*i,cz0,cR,ct,x0+h*j,y0+h*i,z0+h*k);
	}
        if (incl_С<EMPTY_LIMIT)
        V[k][i][j]+=(abs_vel-V[k][i][j])*(incl_C);
      }
