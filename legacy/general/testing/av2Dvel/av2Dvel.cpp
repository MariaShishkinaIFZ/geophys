...
int main(int narg, char ** argv)
{
 ...
 // fclose(fp);  
  ...
  alloc_float_matrix(&V2D,Ny,Nx);
  V2D_av=matrix(1,Ny,1,Nx);  
  ...
  for (j=0; j <Ny; j++)
   ...
   av2d(V2D,V2D_av,Ny,Nx,k_gen); 
   GSTgriddedData *V= new GSTgriddedData(NNy,NNx,y0,(y0+h*k_gen*(NNy)),x0, (x0+h*k_gen*(NNx)),area);
  
   for (j=0; j <NNy; j++)
    for (i=0; i<NNx; i++)
     {
      V->data()[j][i] =V2D_av[j+1][i+1];   
     }
  
    writeGMT( f_2D_av, *V);
    delete V;
   free_float_matrix(V2D,Ny,Nx);
   free_float_3d(VFINE,Nz,Ny,Nx);
   free_matrix(V2D_av,1,Ny,1,Nx);
   fprintf(stderr,"Success.\n");
}
