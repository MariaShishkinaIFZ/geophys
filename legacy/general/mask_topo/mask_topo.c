...
int main(int narg, char **argv)
{
  ...
  float x, y, z;
  float ***Vu, **Int;
  char u_file[FLNLN], int_file[FLNLN];
  char out_file[FLNLN];
  FILE *fp;
  ...
  if (ask) fprintf(stdout,"Input 3D fine grid model filename (%d chars max) > ",FLNLN);
  fscanf(fp,"%s",u_file);
  if (ask) fprintf(stdout,"Input boundary filename (%d chars max) > ",FLNLN);
  fscanf(fp,"%s",int_file);
  if (ask) fprintf(stdout,"Input 3D masked model out file name (%d chars max) > ",FLNLN);
  ...
  for (i=0;i<Ny;i++)
    for (j=0;j<Nx;j++)
      for (k=0;k<(int)ceil((Int[i][j]-z0)/h);k++)
        Vu[k][i][j]=DUMMY_VALUE;
