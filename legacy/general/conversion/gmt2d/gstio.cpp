...
GSTgriddedData *readGMT(char *fileName)
{
  ...
  for (i=H.ny-1;i>=0;i--)
     fread(A->data()[i],sizeof(float),H.nx,fi);
 ...
 
{
  ...
  A.getIntervals(H.y_inc, H.x_inc);
   ...
   H.z_min =(double)A.data()[0][i];
   H.z_max=(double)A.data()[0][i]; 
...
(char *fileName, int M, int N, int *n, char *name, char *divid, int headline=0)
{
  ...
  *divid- разделитель используемый в файле, headline- число срок отведеннок под комментрарий,
  ...
  while (!in.atEnd())   
  {
   ...
   if (cols.size()!=M)
    {
      sprintf(str,"number of columns in line %d, file %s file != M",c,fileName);
      GSTerror(str);
     }
    ...
    for(i=0;i<N;i++)
    {
     ...
     if(!ok)
      {
       sprintf(str,"Data format error in file %s",fileName);
       GSTerror(str);
      }
    }     
   D->pntAdd(A);    
...
void  WriteColumns
(char *fileName, GSTscatterData &D,char *names=NULL,const char *divid=",", int fieldwidth=0)
{
