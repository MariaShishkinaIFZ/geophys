...
// read_3D_volume(float ***Buf, long Nz, long Ny, long Nx, char *filename)
 ...
 fprintf(stderr,"First interpolate sub-interface slowness variation...");
  ...
  alloc_float_matrix(&bot2D_Rcave,Ny, Nx);  //интерполированные значения лучевого покрытия
  for (j=0;j<Ny;j++)
  {
    ...
    for (i=0;i<Nx;i++)
    {
     ...
     /*
      (1.0/(dd*dd))*
	          (Buff_2D_bot_c[jj][ii]*(dd-x)*(dd-y)+
	           Buff_2D_bot_c[jj][ii+1]*x*(dd-y)+
	           Buff_2D_bot_c[jj+1][ii]*(dd-x)*y+
	           Buff_2D_bot_c[jj+1][ii+1]*x*y);*/
    }
  } 
  ...
  alloc_float_3d(&Rcave, Nz, Ny, Nx);  //интерполированные значения лучевого покрытия
...
fprintf(stderr,"111111111111111.\n");
  ...
  for (k=0; k<Nz; k++)
   for (j=0; j<Ny; j++)
    for (i=0; i<Nx; i++)
     {
       if (Rcave[k][j][i]>0 || Rcave[k][j][i]==-1.0 ) Mask_velmod[k][j][i]=Velmod[k][j][i];
