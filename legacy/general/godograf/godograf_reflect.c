...
int main(int narg, char **argv)
{
 ...
 if (narg < 2) errquit("Insufficient parameters.\n");
  ...
  if (ask) fprintf(stdout,"Input boundary mark point x y z, - coordinates > \n");
  ...
  //    x=sqrt((yp[l]-ys)*(yp[l]-ys)+(xp[l]-xs)*(xp[l]-xs));
      x=(yp[l]-ys);
      td=2*zd/v1*sqrt(1+((x*x+4*zd*x*sin(delta))/(4*zd*zd)));
