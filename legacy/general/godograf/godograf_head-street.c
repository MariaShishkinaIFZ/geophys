...
int main(int narg, char **argv)
{
 ...
 if (narg < 2) errquit("Insufficient parameters.\n");
  ...
  for (l=0;l<Np;l++)
  {
      ...
      if( (sqrt((yp[l]-ys)*(yp[l]-ys)+(xp[l]-xs)*(xp[l]-xs))/v1) >= (sqrt((yp[l]-ys)*(yp[l]-ys)+(xp[l]-xs)*(xp[l]-xs))/v1*sin(teta+ffi)+t1d) )   td=sqrt((yp[l]-ys)*(yp[l]-ys)+(xp[l]-xs)*(xp[l]-xs))/v1*sin(teta+ffi)+t1d;
